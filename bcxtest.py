...
def maketx(sourcetx, sourceidx, wifkey, targetaddr, numsatoshi, originalsatoshi):
    ...
    signature = signdata(sourceprivkey, to_sign) + "\x01"
...
if len(sys.argv) != 7:
    ...
    print "YOU ARE ABOUT TO SEND %.8f BCX FROM %s TO %s!" % (numsatoshi / 100000000.0, sourceaddr, targetaddr)
    print "!!!EVERYTHING ELSE WILL BE EATEN UP AS FEES! CONTINUE AT YOUR OWN RISK!!!"
    print "(There are 10000 BCX per BTC in your address, double check!)"
    ...
    client = Client(("seed.bcx.org", 9003))
    client.connect()
